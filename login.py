...
class LoginWindow(Tk):
    ...
    def __init__(self):
        ...
        self.title("登录界面")
        self.geometry("720x610+100+100")    # 加号表示窗体打开时出现的坐标位置，分别代表x坐标和y坐标

        # 加载窗体
        self.setup_UI()
        # 定义变量（定义全局变量，后面的函数都能访问到）
        self.file_path = "."+os.sep+"student_mess"+os.sep+"User.txt"   # 文件路径
    ...
    def setup_UI(self):
        ...
        self.Style01.configure("TButton",font = ("微软雅黑",16,"bold"),foreground = "black")

        # 创建一个Label标签展示图片
        self.Login_image = PhotoImage(file = "."+os.sep+"img"+os.sep+"tu.gif")
        ...
        self.Entry_password = Entry(self, width=15,textvariable = self.var_password,show = "*",style = "TEntry")  # 密码以什么符号显示，使用show方法
        self.Entry_password.pack(side = LEFT,padx = 5,pady = 8)
        # 创建一个按钮    --- 登录
        self.Button_login = Button(self,text = "登录",width = 15,command = self.login)
        self.Button_login.pack(side = LEFT,padx = 70,pady = 5)
    ...
    def load_main(self):
        ...
        self.destroy()
        # 加载新窗体
        main_window = main.MainWindow(self.current_user_list, self.get_now_time())
