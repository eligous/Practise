...
class MainWindow(Tk):
    ...
    def __init__(self, current_user_list, current_time):
        ...
        self.file_path = "." + os.sep + "student_mess" + os.sep + "student_mess.txt"
    ...
    def setup_UI(self):
        ...
        self.Style01.configure("right.TPanedwindow", foreground="deepskyblue")
        self.Style01.configure("TButton", width=10, font=("华文黑体", 15, "bold"))

        # 左边：按钮区域,创建一个容器
        self.Pane_left = PanedWindow(width=160, height=620, style="left.TPanedwindow")
        self.Pane_left.place(x=10, y=10)
        ...
        # 右边：查询、TreeView
        self.Pane_right = PanedWindow(width=740, height=620, style="right.TPanedwindow")
        self.Pane_right.place(x=180, y=10)
        # LabelFrame
        self.LabelFrame_query = LabelFrame(self.Pane_right, text="学生信息查询", width=720, height=70)
        ...
        if not os.path.exists(self.file_path):
            ...
            try:
                with open(file=self.file_path, mode="r", encoding="UTF-8") as fd:
                    ...
                    while current_line:
                        temp_list = current_line.split(",")  # 长字符串分割
    ...
    def load_detail_window(self):
        detail_window = add_detail.add_detail(self.action_flag, self.current_student_list, self.all_student_list)
    ...
    def load_change_password(self):
        this_change_password = editpassword.editpassword(self.current_login_list)
